...
router.get("/:username/scripts", async (req, res) => {
    const username = (req.params.username ?? "").toLowerCase();
    const tab = req.query.tab ?? "scripts";
    ...
    if (tab === "liked") {
        ...
        const ordered = scripts.slice().sort((a, b) => (orderMap.get(a.id) ?? 0) - (orderMap.get(b.id) ?? 0));
        res.json({
    ...
    if (tab === "collections" || tab === "activity") {
        res.json({ scripts: [], total: 0, page: 1, limit });
        return;
    }
...
router.post("/:id/follow", authenticateToken, async (req, res) => {
    ...
    if (existing) {
        ...
        res.json({
            ...
            followingCount: (await prisma.user.findUnique({ where: { id: followerId }, select: { followingCount: true } })).followingCount,
        });
        return;
    }
    await prisma.$transaction([
