...
router.get("/reports", async (req, res) => {
    ...
    res.json({
        reports: reports.map((r) => {
            const script = (r.script ?? r.comment?.script ?? null);
            return {
                id: r.id,
                ...
                script: script
                    ? {
                        ...
                        status: "status" in script ? script.status.toLowerCase() : "testing",
...
router.post("/reports/:id/resolve", async (req, res) => {
    ...
    const authorId = report.script?.authorId ??
...
router.post("/users/:id/shadowban", async (req, res) => {
    const moderatorId = req.user.id;
    const userId = req.params.id;
    await prisma.user.update({
        where: { id: userId },
...
router.post("/scripts/:id/feature", async (req, res) => {
    const moderatorId = req.user.id;
